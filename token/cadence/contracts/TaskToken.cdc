...
access(all) contract TaskToken: FungibleToken {
        ...
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(
                    ...
                    receiverLinkedType: Type<&TaskToken.Vault>(),
                    metadataLinkedType: Type<&TaskToken.Vault>(),
                    createEmptyVaultFunction: (fun(): @{FungibleToken.Vault} {
                        return <-TaskToken.createEmptyVault(vaultType: Type<@TaskToken.Vault>())
                    })
                )
            case Type<FungibleTokenMetadataViews.TotalSupply>():
                return FungibleTokenMetadataViews.TotalSupply(
                    totalSupply: TaskToken.totalSupply
                )
        }
        return nil
    }
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                TaskToken.totalSupply = TaskToken.totalSupply - self.balance
            }
            self.balance = 0.0
        }
        ...
        access(all) view fun getViews(): [Type] {
            return TaskToken.getContractViews(resourceType: nil)
        }

        access(all) fun resolveView(_ view: Type): AnyStruct? {
            return TaskToken.resolveContractView(resourceType: nil, viewType: view)
        }
        ...
        /// elsewhere.
        ///
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @TaskToken.Vault {
        ...
        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @TaskToken.Vault
        ...
        ///
        access(all) fun createEmptyVault(): @TaskToken.Vault {
    ...
    access(all) resource Minter {
        ...
        ///
        access(all) fun mintTokens(amount: UFix64): @TaskToken.Vault {
            TaskToken.totalSupply = TaskToken.totalSupply + amount
    ...
    /// account to be able to receive deposits of this token type.
    ///
    access(all) fun createEmptyVault(vaultType: Type): @TaskToken.Vault {
        return <- create Vault(balance: 0.0)
    }
    ...
    init() {
        ...
        //
        let exampleTokenCap = self.account.capabilities.storage.issue<&TaskToken.Vault>(self.VaultStoragePath)
        self.account.capabilities.publish(exampleTokenCap, at: self.VaultPublicPath)
        let receiverCap = self.account.capabilities.storage.issue<&TaskToken.Vault>(self.VaultStoragePath)
