...
transaction(addressAmountMap: {Address: UFix64}) {
    ...
    let vaultRef: auth(FungibleToken.Withdraw) &TaskToken.Vault

    prepare(signer: auth(BorrowValue) &Account) {
        self.vaultData = TaskToken.resolveContractView(resourceType: nil, viewType: Type<FungibleTokenMetadataViews.FTVaultData>()) as! FungibleTokenMetadataViews.FTVaultData?
            ?? panic("Could not resolve FTVaultData view. The TaskToken"
                .concat(" contract needs to implement the FTVaultData Metadata view in order to execute this transaction."))

        // Get a reference to the signer's stored vault
        self.vaultRef = signer.storage.borrow<auth(FungibleToken.Withdraw) &TaskToken.Vault>(from: self.vaultData.storagePath)
            ?? panic("The signer does not store an TaskToken.Vault object at the path "
